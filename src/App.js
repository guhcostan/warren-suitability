          ...
          <>
                ...
                <Typewriter
                  onInit={(typewriter) => {
                    typewriter
                      .changeDelay(20)
                      .typeString(
                        messages[0].value.replaceAll(/\^[0-9]+/gi, ''),
                      )
                      .typeString(
                        messages[1] &&
                          messages[1].value.replaceAll(/\^[0-9]+/gi, ''),
                      )
                      .typeString(
                        messages[2] &&
                          messages[2].value.replaceAll(/\^[0-9]+/gi, ''),
                      )
                      .typeString(
                        messages[3] &&
                          messages[3].value.replaceAll(/\^[0-9]+/gi, ''),
                      )
                      .callFunction(() => {
                        setRenderInput(true)
                      })
              ...
              {renderInput &&
                inputs.map((input) => (
                  <Input
                    ...
                    key={input.mask}
                    value={answers ? answers[idMessage] : ''}
                    onChange={(e) => {
            ...
            <Buttons>
              {renderInput && buttons.length > 0
                ...
                : renderInput &&
                  !profile && <Button onClick={() => nextStep()}>OK</Button>}
            </Buttons>
          </>
        ) : (
          <Lottie
